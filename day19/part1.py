...
def compute(s: str) -> int:
    ...
    for line in s.strip().splitlines():
        ...
        def find_max(m, ore, clay, obsidian, geode, r_ore, r_clay, r_obsidian, r_geode):
            sig = (m, ore, clay, obsidian, geode, r_ore, r_clay, r_obsidian, r_geode)
            if sig in cache:
                return cache[sig]
            if m == 0:
                return geode
            max_val = find_max(m - 1, ore + r_ore, clay + r_clay, obsidian + r_obsidian, geode + r_geode, r_ore, r_clay, r_obsidian, r_geode)
            sig = (m - 1, ore + r_ore, clay + r_clay, obsidian + r_obsidian, geode + r_geode, r_ore, r_clay, r_obsidian, r_geode)
            cache[sig] = max_val
            ...
            if can_create_r_geode(ore, obsidian):
                max_val = max(max_val, find_max(m - 1, ore + r_ore - geode_robot_ore, clay + r_clay, obsidian + r_obsidian - geode_robot_obs,
                          geode + r_geode, r_ore, r_clay, r_obsidian, r_geode + 1))

            sig = (m, ore, clay, obsidian, geode, r_ore, r_clay, r_obsidian, r_geode)
            cache[sig] = max_val
            return max_val
        max_geode = find_max(24, 0, 0, 0, 0, 1, 0, 0, 0)
        print(len(cache))
        print(max_geode)
        res += i * max_geode
    return res
